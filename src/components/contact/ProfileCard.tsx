...
// NOTE: Styles live in src/app/profile-card.css and are imported globally from layout.tsx
// Do not import CSS here to avoid global CSS import errors in App Router.
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  avatarUrl = "/about-me/photo-1.png",
  iconUrl,
  grainUrl,
  behindGradient,
  innerGradient,
  ...
  const handlePointerLeave = useCallback(
    (event: React.PointerEvent<HTMLElement>) => {
      ...
      // Support both React synthetic events and native events
      const anyEvent: any = event as any;
      const native: any = anyEvent?.nativeEvent ?? anyEvent;

      // Prefer offsetX/offsetY; fallback to clientX/Y relative to the card rect
      let startX = 0;
      let startY = 0;
      if (typeof native?.offsetX === "number" && typeof native?.offsetY === "number") {
        startX = native.offsetX;
        startY = native.offsetY;
      } else {
        const rect = card.getBoundingClientRect();
